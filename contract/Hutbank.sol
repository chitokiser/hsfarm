...
contract hutbank {
  ...
  constructor(address _cya, address _hut, address _hutb) {
    fix = 1e16;  
    cya = Icya(_cya);
    hut = Ihut(_hut);
    bank = _hutb;  // hut 뱅크
  ...
  function bankup(address _bank) public {  
    require(staff[msg.sender] >= 5, "no staff");
    bank = _bank;   // 초기값은 hutbank에 줄 것
  }

  function buyhut(uint _num) public returns(bool) {  
    uint pay = _num * price;
    require(act >= 1, "Not for sale");  
    require(g3() >= _num, "hut sold out");  
...
function sellhut(uint num) public returns(bool) {      
...
function allowcation() public returns(bool) {   // depo 증가
    ...
    require(hut.getdepot(msg.sender) + 7 days < block.timestamp, "cut not time"); // 주 1회
    allowt[msg.sender] = block.timestamp;
